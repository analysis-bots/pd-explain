...
class FedexExplainer(ExplainerInterface):
    ...
    def visualize(self, fedex_output: tuple = None, added_explanations=None) -> None:
        """
    ...
    def get_explanations(self, indexes=None) -> tuple[str, list[dict[str, str]]]:
        """
        ...
        :return: A tuple containing the query and a list of dictionaries with the explanations and any added LLM context.
        """
        if self._results is None:
            raise ValueError("No explanations have been generated yet. Please call generate_explanation() first.")
        title, scores, K, figs_in_row, explanations, bins, influence_vals, source_name, show_scores = self._results
        # Combine the explanations and the added explanations if they exist
        to_return_list = []
        if self._added_explanations is not None:
            for i in explanations.index.values:
                explanation = explanations.loc[i]
                if explanation in self._added_explanations:
                    out = {
                        "explanation": explanation,
                        "added LLM context": self._added_explanations[explanation]["added_text"],
                    }
                else:
                    out = {
                        "explanation": explanation,
                        "added LLM context": None,
                    }
                to_return_list.append(out)
        else:
            to_return_list = [
                {
                    "explanation": explanations.loc[i],
        ...
        textual_description = (f"Running the fedex explainer on the query {self._query} has returned the following explanations, "
                               f"for the most important statistical changes that occurred in the data as a result of the query: ")
        if indexes is not None:
            if isinstance(indexes, int):
                return textual_description, [to_return_list[indexes]]
            elif isinstance(indexes, list):
                return textual_description, [to_return_list[i] for i in indexes if i < len(to_return_list)]
            else:
                raise ValueError("Indexes must be an integer or a list of integers.")
        return textual_description, to_return_list
